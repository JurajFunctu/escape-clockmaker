...
export class LevelCompleteScene extends Phaser.Scene {
  ...
  create(): void {
    ...
    if (this.textures.exists('bg_level1_complete')) {
      const bg = this.add.image(width / 2, height / 2, 'bg_level1_complete');
    ...
    const title = this.add.text(width / 2, height / 2 - 200, 'Level 1 Complete!', {
      fontFamily: FONT,
      fontSize: '52px',
      color: '#ffd700',
      fontStyle: 'bold',
    ...
    const subtitle = this.add.text(width / 2, height / 2 - 120, 'Myška má nový domov!', {
      fontFamily: FONT,
      fontSize: '28px',
      color: '#ffffff',
      stroke: '#333333',
    ...
    nextBtn.fillStyle(0x2196f3, 1);
    nextBtn.fillRoundedRect(width / 2 - 140, nextBtnY - 30, 280, 60, 16);

    this.add.text(width / 2, nextBtnY, 'Ďalší level →', {
      fontFamily: FONT,
      fontSize: '26px',
      color: '#ffffff',
      fontStyle: 'bold',
    ...
    const nextZone = this.add.zone(width / 2, nextBtnY, 280, 60).setInteractive();

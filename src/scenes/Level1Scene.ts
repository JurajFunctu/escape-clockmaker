...
const OBJECT_DEFS: Array<{
  ...
  { id: 'cheese', spriteKey: 'obj_cheese', label: 'Syr', x: 560, y: 520, size: 90, message: 'Mmm, syr! Mám hlad!' },
  { id: 'candle', spriteKey: 'obj_candle', label: 'Sviečka', x: 130, y: 370, size: 100, message: 'Teraz tu bude svetlo!' },
  { id: 'cup', spriteKey: 'obj_cup', label: 'Šálka', x: 420, y: 730, size: 85, message: 'Čajík by bol skvelý!' },
  { id: 'blanket', spriteKey: 'obj_blanket', label: 'Deka', x: 280, y: 850, size: 100, message: 'Na túto deku sa vyspím!' },
  { id: 'key', spriteKey: 'obj_key', label: 'Kľúč', x: 620, y: 290, size: 75, message: 'Kľúč! Teraz je dom môj!' },
];

export class Level1Scene extends Phaser.Scene {
  private tracker!: ObjectTracker;
  private hiddenObjects: HiddenObject[] = [];
  private mouseSprite!: Phaser.GameObjects.Image | Phaser.GameObjects.Container;
  ...
  create(): void {
    ...
    if (this.textures.exists('bg_level1')) {
      const bg = this.add.image(width / 2, height / 2, 'bg_level1');
    ...
    this.createMouseCharacter(width, height);
  ...
  private createFallbackBackground(width: number, height: number): void {
    this.add.rectangle(width / 2, height - 200, width, 400, 0x8b6d4f).setDepth(0);
    this.add.rectangle(width / 2, height / 2 - 200, width, height - 400, 0xd4b896).setDepth(0);
    this.add.rectangle(width / 2, 140, width, 20, 0x5a3e2b).setDepth(0);
    const winG = this.add.graphics().setDepth(0);
    winG.fillStyle(0x87ceeb, 0.6);
    winG.fillRect(80, 350, 140, 180);
    winG.lineStyle(4, 0x5a3e2b, 1);
    winG.strokeRect(80, 350, 140, 180);
    this.add.rectangle(580, 640, 200, 12, 0x5a3e2b).setDepth(0);
    this.add.rectangle(400, 930, 300, 15, 0x6d4c2e).setDepth(0);
  }
  ...
  private createMouseCharacter(width: number, _height: number): void {
    const mouseX = width / 2;
    const mouseY = 1050;

    if (this.textures.exists('char_mouse')) {
      const mouse = this.add.image(mouseX, mouseY, 'char_mouse');
      mouse.setDisplaySize(220, 220);
      mouse.setDepth(100);
      this.mouseSprite = mouse;
    } else {
      const container = this.add.container(mouseX, mouseY);
      container.setDepth(100);
      container.setScale(2.0);
      const body = this.add.ellipse(0, 0, 80, 90, 0xbbbbbb);
      container.add(body);
      const earL = this.add.circle(-25, -40, 18, 0xdddddd);
      const earR = this.add.circle(25, -40, 18, 0xdddddd);
      container.add(earL);
      container.add(earR);
      const earLI = this.add.circle(-25, -40, 10, 0xffaaaa);
      const earRI = this.add.circle(25, -40, 10, 0xffaaaa);
      container.add(earLI);
      container.add(earRI);
      const eyeL = this.add.circle(-14, -12, 6, 0x000000);
      const eyeR = this.add.circle(14, -12, 6, 0x000000);
      container.add(eyeL);
      container.add(eyeR);
      const nose = this.add.circle(0, 0, 5, 0xff9999);
      container.add(nose);
      this.mouseSprite = container;
    }
    ...
    this.createSpeechBubble(mouseX, mouseY - 150);
  }
  ...
  private onAllFound(): void {
    ...
    this.time.delayedCall(1000, () => {
      this.showSpeech('Ďakujem! Toto je môj nový domov! 🏠');
      this.createConfetti();
